...
// The argument parser type.
// This is used to prevent the user from interfering with the builtin argument parsers.
enum argument_parser_source_t {
    // The argument parser may be any type
    // This is only used for requesting a type
    argument_parser_source_any,
    
    // The argument parser is a builtin
    argument_parser_source_builtin,
    
    // The argument parser is user-supplied
    argument_parser_source_user_supplied
};
...
class argument_parser_set_t {
    friend class doc_register_t;
    friend bool register_argument_parser(const wcstring &, enum argument_parser_source_t, const wcstring &, const wcstring &, parse_error_list_t *);
    std::vector<shared_ptr<const docopt_registration_t> > registrations;
    
    /** Returns a subset of the receiver matching the given source */
    argument_parser_set_t filter_by_source(enum argument_parser_source_t source) const;
...
/** Given a command, name, usage spec, and description, register the usage. If cmd is empty, infers
 ...
 \param source The source from which this parser is derived
 ...
 */
bool register_argument_parser(const wcstring &cmd, enum argument_parser_source_t source, const wcstring &usage,
                              const wcstring &description, parse_error_list_t *out_errors);

/** Get the set of registrations for a given command */
argument_parser_set_t docopt_get_registrations(const wcstring &cmd,
                                               enum argument_parser_source_t source = argument_parser_source_any);
