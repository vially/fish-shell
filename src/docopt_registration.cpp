...
/* Weird function. Given a parser status and an existing argument status, convert the parser status
 * to the argument status and return the "more valid" of the two. This supports our design for
 * multiple parsers, where if any parser declares an argument valid, that argument is marked valid.
 */
enum docopt_argument_status_t more_valid_status(docopt_fish::argument_status_t parser_status,
                                                docopt_argument_status_t existing_status) {
    enum docopt_argument_status_t new_status =
        static_cast<enum docopt_argument_status_t>(parser_status);
    switch (existing_status) {
        case status_invalid:
            return new_status;
        case status_valid:
            return status_valid;
        case status_valid_prefix:
            return new_status == status_valid ? new_status : existing_status;
        default:
            assert(0 && "Unknown argument status");
...
std::vector<docopt_argument_status_t> docopt_registration_set_t::validate_arguments(
    ...
    // For each parser, have it validate the arguments. Mark an argument as the most valid that any
    // parser declares it to be.
    for (size_t i = 0; i < registrations.size(); i++) {
        const docopt_parser_t *p = registrations.at(i)->parser.get();
        const std::vector<docopt_fish::argument_status_t> parser_statuses =
            p->validate_arguments(argv, flags);
...
bool docopt_registration_set_t::parse_arguments(const wcstring_list_t &argv,
    ...
    for (size_t i = 0; i < registrations.size(); i++) {
        ...
        docopt_argument_map_t args = registrations.at(i)->parser->parse_arguments(
            argv, docopt_fish::flags_default, &errors, (wants_unused ? &local_unused_args : NULL));

        // Insert values from the argument map. Don't overwrite, so that earlier docopts take
        // precedence
...
wcstring docopt_arguments_t::dump() const {
    wcstring result;
    for (std::map<wcstring, wcstring_list_t>::const_iterator iter = this->vals.begin();
         iter != this->vals.end(); ++iter) {
...
void docopt_arguments_t::swap(docopt_arguments_t &rhs) { this->vals.swap(rhs.vals); }
